...
public class ControllerTest {
    ...
    //  요청을 처리해주는 webApplicationContext 호출
    @Autowired
    private WebApplicationContext webApplicationContext;

    //  하위의 모든 테스트가 실행 전에 해당 메소드 우선 실행
    @BeforeEach
    public void setUp(){
    ...
    public void testRemove() throws Exception{
        String result = mockMvc.perform(MockMvcRequestBuilders.post("/board/remove").param("bno", "5")).andReturn().getFlashMap().toString();
        log.info(result);
    }
